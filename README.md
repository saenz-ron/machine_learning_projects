# Machine Learning Projects

Author: Ronnie Saenz

Description: These are a variety of small scale Machine Learning projects that demonstrate various applications of Machine Learning.

## Table of Contents:

### Project 1: Gradient Descent
This project uses gradient descent to find the minimum for the function f(x,y,z) = x^2+y^2+z^2-x-y+z. In addition, the script displays a curve that illustrates the change of the values of (x, y, z) between two consecutive iteration changes with respect to the iteration number.

### Project 2: Eigendecomposition
This project demonstrates the process of eigendecomposition. It starts with a matrix A and obtains both square matrices that can be constructed from A. For each of the two matrices, the program obtains P, D, and P^-1 and confirms that the sum of these three matrices equals the original matrix. Each step of the computation is displayed on screen.

### Project 3: Pseudo-Random Number Generators (PRNGS)
This project generates M pseudo-random numbers from distribution U(0,1) and finds their sum. This is repeated N times, and after the iterations, a plot is created that illustrates their distribution. Another plot is created using normal distribution from the normal distribution generator from numpy.

### Project 4: Synthetic Datasets


### Project 5: Linear Regression
This project performs linear regression on a linearized dataset that originally can be assumed to follow the exponential model y = a*e^(bx). The value for a is 7, the value for b is user inputted, and the value for x is a random value between 0 and 5. A data set of 100 observations is generated with Gaussian noise added to each compound value of y. The dataset is displayed then linearized. Finally, the script performs linear regression on the dataset, then prints the estimated values of a and b with a comparison to the original values.

### Project 6: Multiple Regression
This project creates a noisy dataset based on a linear combination pattern and uses multiple regression to discover the pattern. 

### Project 7: Unsupervised Learning 1

### Project 8: Unsupervised Learning 1

### Project 9: Neural Network 1

### Project 10: Neural Network 2

